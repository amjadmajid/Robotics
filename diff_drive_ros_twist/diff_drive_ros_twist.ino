...
void moveCb(geometry_msgs::Twist& msg){
  digitalWrite(LED_BUILTIN, HIGH-digitalRead(LED_BUILTIN));
  if (msg.angular.z == 0 ){
      if (msg.linear.x <= 140){
          m_1.speed = msg.linear.x;
          m_2.speed = msg.linear.x; 
      }else{
          m_1.speed = 140;
          m_2.speed = 140; 
      }
...
ros::Subscriber<geometry_msgs::Twist> moveSub("cmd_vel", &moveCb);

//------------ wheels ticks publishers 

std_msgs::Int16 right_wheel_ticks_cntr;
ros::Publisher rightWheelPub("right_ticks", &right_wheel_ticks_cntr);

std_msgs::Int16 left_wheel_ticks_cntr;
ros::Publisher leftWheelPub("left_ticks", &left_wheel_ticks_cntr);
...
void loop() {
  ...
  right_wheel_ticks_cntr.data = m_1.tacho.ticks;
  left_wheel_ticks_cntr.data = m_2.tacho.ticks;
  nh.spinOnce();

  if(current_millis - prev_millis >= LOOPTIME){
      rightWheelPub.publish(&right_wheel_ticks_cntr);
      leftWheelPub.publish(&left_wheel_ticks_cntr);
